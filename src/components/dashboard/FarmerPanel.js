...
export default function FarmerPanel({ user }) {
  ...
  const [form, setForm] = useState({ titleAz: "", price: "", stock: 1, categoryId: "", region: "", city: "", descriptionAz: "", images: [], isCorporate: false, minOrderQty: "", tags: [] });
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        minOrderQty: form.isCorporate && form.minOrderQty ? parseInt(form.minOrderQty) : null,
        tags: form.tags || [],
      };
      await apiFetch("/api/products", { method: "POST", body: JSON.stringify(payload) });
      setMsg("Elan yaradıldı! Admin təsdiqindən sonra aktivləşəcək.");
      setForm({ titleAz: "", price: "", stock: 1, categoryId: "", region: "", city: "", descriptionAz: "", images: [], isCorporate: false, minOrderQty: "" });
      loadMyProducts();
    } catch (err) {
  ...
  function startEdit(p) {
    setEditingId(p.id);
    setEditForm({ titleAz: p.titleAz || p.title, price: p.price, stock: p.stock, region: p.region || "", city: p.city || "", images: p.images || (p.coverImage ? [{ url: p.coverImage }] : []) });
  }
